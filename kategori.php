...
$kategori		=	veriCek("category_rows", "*", "seo_name", $kategori_seo);

if( !$kategori )
	git("/");
	
$kategori_up	=	veriCek("category_name", "*", "id", $kategori["target"]);
?>
<!DOCTYPE HTML>
<html lang="en-US">
	<head>
		<base href="<?=$site_url?>"  />
		<?=$site->head($kategori_up["name"] . " - " . $kategori["name"]);?>
	...
	<body>
		...
		<div class="container">
			<div class="edc col-md-8">
				...
				<?php
					$users_posts = tabloCek("users_posts", "*", "WHERE category LIKE '%{$kategori["id"]}%' ORDER BY id DESC");
					...
					$users_posts = tabloCek("users_posts", "*", "WHERE category LIKE '%{$kategori["id"]}%' ORDER BY id DESC LIMIT {$limit}, {$goruntulenecekToplamItem}");
				...
				?>
					<nav aria-label="Page navigation">
						<ul class="pagination">
							...
							if($sayfa != 1) echo '<li><a href="/kategori/'.$kategori["seo_name"].'/1">İlk</a></li>';

							for($s = $sol_sayfalar; $s <= $sag_sayfalar; $s++) {
								if($sayfa == $s) {
									echo "<li class='active'><a href='javascript:void(0)'>{$s}</a></li>"; 
								} else {
									echo "<li><a href='/kategori/{$kategori["seo_name"]}/{$s}'>{$s}</a></li>";
								}
							}
							
							if($sayfa != $toplam_sayfa) echo "<li><a href='/kategori/{$kategori["seo_name"]}/{$toplam_sayfa}'>Son</a></li>";
							?>
						</ul>
					</nav>
				<?php } ?>
